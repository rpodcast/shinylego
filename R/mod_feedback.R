...
mod_feedback_server <- function(input, output, session){
  ...
  observeEvent(input$submit_issue, {
    ...
    # submit issue
    res <- submit_issue(input$issue_title, input$issue_labels, input$issue_description)
    
    # show confirmation
    shinyWidgets::sendSweetAlert(
      session = session,
      title = "Issue Submitted!",
      text = "Thank you for your feedback! Your issue has been submitted to the MOATR issue tracker. One of the maintainers will review your issue and contact you for additional details.",
      type = "success"
    )
